...
export const QuestionEdit: React.FC<Props> = ({
    ...
    const [a, b] = useState<number>(
        question.options.findIndex((option) => question.expected === option)
    );
    ...
    const handleSwitch = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (newType !== question.type) {
            if (newType === "multiple_choice_question") {
                switchMulti();
            } else if (newType === "short_answer_question") {
    ...
    const handleNumOptions = (e: React.ChangeEvent<HTMLInputElement>) => {
        b(0);
        const newNum = Math.max(1, parseInt(e.target.value));
        editQuestion(question.id, {
            ...question,
    ...
    const handleRadioChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const updatedQuestion = {
            ...question,
            expected: question.options[parseInt(e.target.value)]
        };
        editQuestion(question.id, updatedQuestion);
    };
